// Pure query/filter/stats logic over a plain project list. Deliberately
// data-agnostic so the in-app module (src/api/projects.js) and the
// Cloudflare Worker (worker/index.js) share one implementation instead of
// two copies that can drift apart — the Worker runs over a JSON snapshot of
// the same data rather than the React-bundled src/data/projects.js.
...
/** Parses a query string, URLSearchParams, or plain object into a filters object for queryProjects. */
...
/** Builds a bound API — queryProjects/getProject/getFilterOptions/getProjectStats — over `projectsData`. */
export function createProjectsApi(projectsData) {
   ...
   */
  function queryProjects(filters = {}) {
   ...
   */
  function getProjectStats(filters = {}) {
  ...
  return { queryProjects, getProject, getFilterOptions, getProjectStats, parseFilterParams };
}
